...
---------------------------------------------------------------------------------------------------------------------
- MODIFIED VERSION
- SOURCCE: https://github.com/alistairewj/sepsis3-mimic/blob/master/query/tbls/cohort.sql
...
select
  ...
  , case when t1.suspected_infection_time_poe is not null  -- CHANGED FROM ORIGINAL!
      ...
      else 0 end as inclusion_suspicion_after_4_hours
  ...
  , case when
          ...
          --or (  -- CHANGED FROM ORIGINAL, i commented this criterion out!
          --        t1.suspected_infection_time_poe is not null
