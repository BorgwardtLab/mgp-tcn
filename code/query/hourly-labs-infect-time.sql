/*
Determine sepsis-relevant laboratory extreme values of the last 24 hours on a hourly base
---------------------------------------------------------------------------------------------------------------------
- MODIFIED VERSION
- SOURCE: https://github.com/alistairewj/sepsis3-mimic/blob/master/query/tbls/labs-infect-time.sql
- AUTHOR (of this version): Michael Moor, February 2018
---------------------------------------------------------------------------------------------------------------------
*/

-- This query pivots lab values taken in the patient's ICU stay (hourly extracting the extreme of the last 24h)
-- don't worry about the itemid's all being around 50k! there is a seperate labitems.csv! (they all range around 50k independently of mv/cv)
-- Have already confirmed that the unit of measurement is always the same: null or the correct unit

DROP FUNCTION IF EXISTS get_labs(int);
CREATE FUNCTION get_labs(int) returns void
    AS 
    $$
...
( -- begin query that extracts the data
 ...
 --     when itemid = 51144 and valuenum > 100 then null -- immature band forms, %
  ...
  ( select s.*, ie.hadm_id from suspinfect_poe s  --CHANGED FROM ORIGINAL: added _poe!
    inner join icustays ie
    on s.icustay_id = ie.icustay_id ) ie
  left join labevents le
    on le.hadm_id = ie.hadm_id
    and le.charttime
--    between (ie.si_starttime + $1 * interval '1 hour') - interval '72' hour  -- try 3d sliding window!
--        and ie.si_starttime + $1 * interval '1 hour'    
--     between ie.si_starttime - interval '24' hour -- try out new 1d window but hardcoded!
--        and ie.si_starttime    
--      between ie.si_starttime -- try out old 3d window!
--       and ie.si_endtime  
    between (ie.si_starttime + $1 * interval '1 hour') - interval '24' hour  
        and ie.si_starttime + $1 * interval '1 hour'    
      and le.ITEMID in
      (
